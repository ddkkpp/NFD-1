...
{
  static const auto strategyName=Name("/localhost/nfd/strategy/malmulticast/%FD%03").appendVersion(4);
  return strategyName;
}

void
MalMulticastStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
MalMulticastStrategy::afterReceiveData(const Data& data, const FaceEndpoint& ingress, 
                  const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MalMulticastStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                                   const shared_ptr<pit::Entry>& pitEntry)
{
