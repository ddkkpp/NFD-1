...
{  
  ...
  //找到收到数据端口的fib nexthop
  const fib::Entry& fibEntry=m_fib.findLongestPrefixMatch(data.getName());
  fib::NextHopList& nexthops = const_cast<fib::NextHopList&>(fibEntry.getNextHops());
  auto it=nexthops.end();
  it = std::find_if(nexthops.begin(), nexthops.end(), [&] (auto& nexthop) {
    return nexthop.getFace().getId()==ingress.face.getId();
  });
  ...
  if(p<0.1){
    if(data.getTag<ndn::lp::ExtraDelayTag>()!=nullptr){
          data.setTag(make_shared<ndn::lp::ExtraDelayTag>(4+*(data.getTag<ndn::lp::ExtraDelayTag>())) );
    }
    else{
      data.setTag(make_shared<ndn::lp::ExtraDelayTag>(4) );
    }
    if(data.getSignatureInfo().getSignatureType()==1){
      auto time=(ns3::Simulator::Now()-verifyTime).GetMilliSeconds();
      verifyTime=ns3::Simulator::Now();
      NFD_LOG_DEBUG("经过时间: "<<time);
      auto rank_change=double(2) / double(1+exp(-time/1000))-1;
      NFD_LOG_DEBUG("rank_change: "<<rank_change);
