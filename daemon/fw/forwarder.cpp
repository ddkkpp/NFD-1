...
void detectWDCallback(Forwarder *ptr)
{
    ...
    {
       NFD_LOG_INFO("seq= "<<it->first);
       double temp = double(it->second.size()) / double((ptr->n).size());
       NFD_LOG_INFO("user number= "<<it->second.size()<<" total user number= "<<(ptr->n).size());
       NFD_LOG_INFO("ratio of user number= "<<temp);
       if(ptr->wdCount==1){
        ...
        NFD_LOG_INFO("rho= "<<ptr->rho[it->first]);
    ...
    NFD_LOG_INFO("mean_rho= "<<mean_rho<<" sigma_rho= "<<sigma_rho);

    mean_r = sum_r / double(ptr->n_u.size());
    sigma_r = sqrt(sum2_r / double(ptr->n_u.size()) - mean_r * mean_r);
    NFD_LOG_INFO("mean_r= "<<mean_r<<" sigma_r= "<<sigma_r);

    mean_av = sum_av / double(ptr->n_u.size());
    sigma_av = sqrt(sum2_av / double(ptr->n_u.size()) - mean_av * mean_av);
    NFD_LOG_INFO("mean_av= "<<mean_av<<" sigma_av= "<<sigma_av);

    if(ptr->wdCount == 1)
    {
        ptr->thr_av = mean_av + ptr->timesOfStdOfav * sigma_av;
        ptr->thr_r = mean_r + ptr->timesOfStdOfr * sigma_r;
        ptr->thr_rho = mean_rho + ptr->timesOfStdOfrho * sigma_rho;//rho计算方式不一样
    }
    else
    {
        ptr->thr_av = ptr->lambda * (mean_av + ptr->timesOfStdOfav * sigma_av) + (1 - ptr->lambda) * ptr->thr_av;
        ptr->thr_r = ptr->lambda * (mean_r + ptr->timesOfStdOfr * sigma_r) + (1 - ptr->lambda) * ptr->thr_r;
        ptr->thr_rho = ptr->lambda * (mean_rho + ptr->timesOfStdOfrho * sigma_rho) + (1 - ptr->lambda) * ptr->thr_rho;
    }
    NFD_LOG_INFO("thr_av= "<<ptr->thr_av);
    NFD_LOG_INFO("thr_r= "<<ptr->thr_r);
    NFD_LOG_INFO("thr_rho= "<<ptr->thr_rho);
    ...
    for(auto it = ptr->n_u.begin(); it != ptr->n_u.end(); it++)
    {
        if(av[it->first] > ptr->thr_av)
        {
            ...
            NFD_LOG_INFO("detect seq= "<<it->first<<" is malicious, av= "<<av[it->first]);
        }
        // if((r[it->first] > ptr->thr_r) && (ptr->rho[it->first] < ptr->thr_rho))
        // {
        //     ptr->malicious.insert(it->first);
        //     NFD_LOG_INFO("detect seq= "<<it->first<<" is malicious, r= "<<r[it->first]<<" rho= "<<ptr->rho[it->first]);
        // }
