...
//此处决定是否缓存
  ...
  for (const auto& pitEntry : pitMatches) {
    ...
    {
      if(data.getSignatureInfo().getSignatureType()==2){
        return;//丢弃
      }
    }     
  ...
  else if(nodeType==verifyNode)
  {
    ++verificationTimes;
    //加验证延时
    //NFD_LOG_DEBUG("添加验证时延tag");
    if(data1->getTag<ndn::lp::ExtraDelayTag>()!=nullptr){
      //NFD_LOG_DEBUG("原始ExtraDelayTag: "<<*(data1->getTag<ndn::lp::ExtraDelayTag>()));
          data1->setTag(make_shared<ndn::lp::ExtraDelayTag>(8+*(data1->getTag<ndn::lp::ExtraDelayTag>())) );//一个节点验证当作两个节点验证开销
      //NFD_LOG_DEBUG("现在ExtraDelayTag: "<<*(data1->getTag<ndn::lp::ExtraDelayTag>()));
    }
    else{
      data1->setTag(make_shared<ndn::lp::ExtraDelayTag>(8) );
    }
    ...
    for (const auto& downstream : satisfiedDownstreams) {
      ...
      //getScheduler().schedule(ndn::time::milliseconds(8), [=] { onOutgoingData(*data1, *downstream.first);});//一个节点验证当作两个节点验证开销

      this->onOutgoingData(*data1, *downstream.first);//如果真的延时发送，运行会很慢
    }
    return;
  }
  //下游节点忽视假包，不做区块链相关处理
      ...
      if(data1->getSignatureInfo().getSignatureType()==2){
        signatureInfo1->setSignatureType(static_cast< ::ndn::tlv::SignatureTypeValue>(3));
        ...
        // maliciousFace=ingress.face;
          //ingress.hasdetected=true;
        //}
      }
      else{
        // hasDetectedFake=false;
      }
  }
...
void Forwarder::removefib(nfd::fib::Entry& entry, Face& face){
  //NFD_LOG_DEBUG("removefib");
  auto k=m_fib.removeNextHop(entry, face);
  switch (k)
  {
  case nfd::fib::Fib::RemoveNextHopResult(0):
    //NFD_LOG_DEBUG("NO_SUCH_NEXTHOP");
    break;
  case nfd::fib::Fib::RemoveNextHopResult(1):
    //NFD_LOG_DEBUG("NEXTHOP_REMOVED");
    break;
  case nfd::fib::Fib::RemoveNextHopResult(2):
    //NFD_LOG_DEBUG("FIB_ENTRY_REMOVED");
    break;
  default:
    //NFD_LOG_DEBUG("none");
    break;
