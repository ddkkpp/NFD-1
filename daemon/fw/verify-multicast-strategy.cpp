...
#include "verify-multicast-strategy.hpp"
...
NFD_REGISTER_STRATEGY(VerifyMulticastStrategy);

NFD_LOG_INIT(VerifyMulticastStrategy);

const time::milliseconds VerifyMulticastStrategy::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds VerifyMulticastStrategy::RETX_SUPPRESSION_MAX(250);

VerifyMulticastStrategy::VerifyMulticastStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    NDN_THROW(std::invalid_argument("VerifyMulticastStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    NDN_THROW(std::invalid_argument(
      "VerifyMulticastStrategy does not support version " + to_string(*parsed.version)));
  }
  this->setInstanceName(makeInstanceName(name, getStrategyName()));
}

const Name&
VerifyMulticastStrategy::getStrategyName()
{
  static const auto strategyName = Name("/localhost/nfd/strategy/verify-multicast").appendVersion(4);
  return strategyName;
}

void
VerifyMulticastStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
VerifyMulticastStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                                   const shared_ptr<pit::Entry>& pitEntry)
{
...
NodeType
VerifyMulticastStrategy::satisfyInterest(const shared_ptr<pit::Entry>& pitEntry,
